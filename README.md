...
> ⚠️ **网页版传大文件的注意事项**：浏览器会把后台标签页节流甚至冻结，导致发送中断。传大文件（>500MB）时请：
> - 把发送页面**单独放一个窗口并保持前台**（可以缩到角落，别最小化、别切到别的标签页后面）；
> - Chrome 可在 设置→性能→"始终保持活动状态" 中加入本站；
> - 或改用下面的 `send.mjs` 命令行版（终端挂着即可，无后台限制，大文件更稳）。
